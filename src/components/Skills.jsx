      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {skillCategories.map((category, idx) => (
            <div key={idx} className="p-8 border border-secondary/10 hover:border-primary transition-colors">
              <h3 className="font-serif text-2xl font-bold text-primary mb-6">{category.title}</h3>
              <div className="flex flex-wrap gap-3">
                {category.skills.map((skill, sIdx) => (
                  <span key={sIdx} className="px-4 py-2 bg-secondary/5 text-secondary font-semibold text-sm hover:bg-primary hover:text-white transition-colors">
                    {skill}
                  </span>
                ))}
              </div>
